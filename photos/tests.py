...
class PhotoListViewTestCase(TestCase):
    def test_photo_list_view(self):
        response = self.client.get(reverse('photos:photo-list'))
...
class PhotoDetailViewTestCase(TestCase):
    ...
    def test_photo_detail_view(self):
        response = self.client.get(reverse('photos:photo-detail', kwargs={'pk': 1}))
        self.assertEqual(response.status_code, 200)
...
class PhotoCreateViewTestCase(TestCase):
    def test_photo_create_view(self):
        response = self.client.get(reverse('photos:photo-create'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'photo/photo_form.html')
...
class PhotoUpdateViewTestCase(TestCase):
    ...
    def test_photo_update_view(self):
        response = self.client.get(reverse('photos:photo-update', kwargs={'pk': 1}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'photo/photo_form.html')
...
class PhotoDeleteViewTestCase(TestCase):
    ...
    def test_photo_delete_view(self):
        response = self.client.get(reverse('photos:photo-delete', kwargs={'pk': 1}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'photo/photo_confirm_delete.html')
...
class FavoritePhotoListViewTestCase(TestCase):
    ...
    def test_favoritephoto_list_view(self):
        response = self.client.get(reverse('photos:favoritephoto-list'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'photo/favoritephoto_list.html')
        self.assertContains(response, "Fotos favoritas")
    ...
    def test_favoritephoto_list_view_no_favorite(self):
        ...
        self.assertTemplateUsed(response, 'photo/favoritephoto_list.html')
        self.assertContains(response, "Fotos favoritas")
        self.assertContains(response, "No tienes ninguna foto como favorita.")
...
class AddToFavoriteViewTestCase(TestCase):
    ...
    def test_add_to_favorites_view(self):
        ...
        response = self.client.get(reverse('photos:add-to-favorites', args=[self.photo.id]))
        self.assertEqual(response.status_code, 302)  # Should be a redirect
        self.assertEqual(response.url, reverse('photos:photo-detail', args=[self.photo.id]))
...
class RemoveFromFavoritesViewTestCase(TestCase):
    ...
    def test_remove_from_favorites_view(self):
        self.client.force_login(self.user)
        response = self.client.get(reverse('photos:remove-from-favorites', args=[self.favorite.id]))
        self.assertEqual(response.status_code, 302)  # Should be a redirect
        self.assertEqual(response.url, reverse('photos:photo-detail', args=[self.photo.id]))
